...
`ifdef FORMAL
    // Abstract the instruction stream as a free symbolic input for formal: the
    // mask-stack/decode/trap properties hold for ANY instruction stream, so we cut the
    // 512-flop I-mem out of the proof's cone of influence (DECISIONS D4.2).
    (* anyseq *) wire [15:0] instr;
`else
...
`endif
    ...
    always_comb begin
`ifdef FORMAL
        // Read path not modeled in formal: decouples I-mem/RF from outputs so opt_clean
        // prunes them from the proof state (they're not in the properties' COI). D4.2.
        csr_rdata = 16'b0;
`else
        case (csr_addr)
...
`ifdef FORMAL
    // ---- Phase 4 formal properties (SymbiYosys) -------------------------------
    // Environment assumption (real host protocol): no CSR writes while the core runs.
    // This excludes the unphysical "reset-core during execution" race from the proof.
    reg f_past;
    initial f_past = 1'b0;
    always_ff @(posedge clk) f_past <= 1'b1;

    // Force a reset in the initial state so the synchronous reset defines all registers
    // (formal starts registers symbolic otherwise). D4.1.
    always_comb if ($initstate) assume (!rst_n);

    always_ff @(posedge clk) begin
        assume (!(csr_we && running));

        if (rst_n) begin
            // (A) mask-stack pointer never exceeds DEPTH — no silent overflow corruption.
            assert (sp <= DEPTH[2:0]);

            if (f_past && $past(rst_n) && $past(do_issue)) begin
                // (B) during execution the stack pointer moves by at most one per cycle.
                assert (sp <= $past(sp) + 2'd1);
                assert ($past(sp) <= sp + 2'd1);

                // (E) decode completeness / forward progress: every issued instruction
                //     either retires or traps — never silently nothing (no X / no hang).
                assert ((c_ret != $past(c_ret)) || trapped);

                // (C) overflow: SPLIT on a full stack traps (overflow) and does NOT push.
                if (($past(op) == OP_SPLIT) && ($past(sp) == DEPTH[2:0]))
                    assert (trapped && (trapcode == TRAP_STACK_OVERFLOW) && (sp == DEPTH[2:0]));

                // (D) underflow: JOIN on an empty stack traps (underflow) and stays at 0.
                if (($past(op) == OP_JOIN) && ($past(sp) == 3'd0))
                    assert (trapped && (trapcode == TRAP_STACK_UNDERFLOW) && (sp == 3'd0));
            end
        end
    end
`endif
